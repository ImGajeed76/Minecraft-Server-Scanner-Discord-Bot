...
const manager = new ShardingManager('./bot.ts', { token: config.token });
let shards = [];
...
async function updateCount() {
  ...
  if (typeof result == 'number')
    for (const shard of shards) shard.send({ type: 'updateCount', count: result });
}

async function updateBedrockCount() {
  let result = await (await fetch(`${config.api}/bedrockCount`)).json();
  if (typeof result == 'number')
    for (const shard of shards) shard.send({ type: 'updateBedrockCount', count: result });
}
