...
function createEmbed(server) {
  ...
  try {
    description2 = JSON.parse(server.rawDescription2);
  } catch (err) {
    description2 = server.description2;
  }
...
function createButtons(server) {
  ...
  buttons.addComponents(
    new ButtonBuilder()
      ...
      .setURL(
        `${config.displayApi || config.api}/bedrockServers?ip=${server.ip}&port=${server.port}`
      )
  );
  return buttons;
}
...
const displayVersion = (version) =>
  `${shortenString(String(version?.name), 19)} (${version?.protocol})`;
...
function createList(servers, currentEmbed, totalResults, minimal) {
  ...
  for (let i = 0; i < servers.length; i++) {
    ...
    if (!minimal)
      description += ` \`${displayVersion(servers[i].version)}${' '.repeat(longest.version - displayVersion(servers[i].version).length)}\` <t:${servers[i].lastSeen}:R>`;
  }
...
export async function buttonHandler(interaction) {
  ...
  switch (id) {
    case 'info': {
      ...
      const server = (
        await (await fetch(`${config.api}/bedrockServers?ip=${ip}&port=${port}`)).json()
      )[0];
      await interaction.reply({
        embeds: [createEmbed(server)],
        components: [createButtons(server)],
      });
      break;
    }
  }
...
export async function execute(interaction: ChatInputCommandInteraction, buttonCallbacks) {
  ...
  function createListButtons(totalResults) {
      ...
      if (`${config.api}/bedrockServers?limit=10&skip=${currentEmbed}&${args}`.length <= 512) {
        buttons.addComponents(
          new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Link)
            .setURL(
              `${config.displayApi || config.api}/bedrockServers?limit=10&skip=${currentEmbed}&${args}`
            )
        );
      }
    ...
    if (totalResults > 10) {
      ...
      buttonCallbacks[lastResultID] = async (interaction) => {
        ...
        servers = (await (
          await fetch(`${config.api}/bedrockServers?limit=10&skip=${currentEmbed}&${args}`)
        ).json()) as any[];
        updateButtons();
        let newEmbed = createList(servers, currentEmbed, totalResults, minimal);
        await interaction.editReply({
          embeds: [newEmbed],
          components: [buttons].concat(infoButtons),
        });
      };
  ...
  if (description != null) {
    ...
    if (
      ...
      args.append('description', segments.join(''));
  }
  ...
  if (excludeRange != null) {
    for (let range of excludeRange.split(',')) {
      ...
      let ipNum = parseInt(ip);
      let subnetNum = subnet == null ? 32 : parseInt(subnet);
      let excludeMinIp = (ipNum & ~((1 << (32 - subnetNum)) - 1)) >>> 0;
      let excludeMaxIp = (ipNum | ((1 << (32 - subnetNum)) - 1)) >>> 0;
      args.append(
        'minIp',
        JSON.stringify(
          [
  ...
  if (servers.length > 0) {
    ...
    if (servers.length == 10)
      new Promise(async (resolve) =>
        resolve(await (await fetch(`${config.api}/bedrockCount?${args}`)).json())
