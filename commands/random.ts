...
function timeSinceDate(date1) {
  ...
  let date2 = new Date();
  let date1Total =
    date1.getSeconds() + date1.getMinutes() * 60 + date1.getHours() * 3600 + date1.getDay() * 86400;
  let date2Total =
    date2.getSeconds() + date2.getMinutes() * 60 + date2.getHours() * 3600 + date2.getDay() * 86400;
...
function createEmbed(server, currentEmbed, totalResults) {
  ...
  const newEmbed = new EmbedBuilder()
    .setColor('#02a337')
    .setTitle(
      `Server ${thousandsSeparators(currentEmbed + 1)}/${thousandsSeparators(totalResults)}`
    )
    .setAuthor({
  ...
  if (server.geo?.country == null) newEmbed.addFields({ name: 'Country: ', value: 'Unknown' });
    ...
    newEmbed.addFields({
      name: 'Country: ',
      value: `:flag_${server.geo.country.toLowerCase()}: ${server.geo.country}`,
    });
  ...
  newEmbed.addFields({
    name: 'Whitelist',
    value:
      server.whitelisted == true ? 'Enabled' : server.whitelisted == false ? 'Disabled' : 'Unknown',
  });
  return newEmbed;
}
...
export async function execute(
  ...
  let lastButtonPress = new Date();
  const randomizeID = `randomize${interaction.user.id}`;
  const oldPlayersID = `oldPlayers${interaction.user.id}`;
  // Status message
  const interactionReplyMessage = await interaction.editReply({
    content: 'Getting a server, please wait...',
    embeds: [],
    components: [],
  });
  ...
  if (recentServers == null)
    recentServers = await (
      await fetch(`${config.api}/count?seenAfter=${Math.round(new Date().getTime() / 1000) - 3600}`)
    ).json();
  let index = Math.floor(Math.random() * recentServers);
  const server = (
    await (
      await fetch(
        `${config.api}/servers?limit=1&skip=${index}&seenAfter=${Math.round(new Date().getTime() / 1000) - 3600}`
      )
    ).json()
  )[0];
  let playerList;
  ...
  if (server == null) {
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        'This is a bug, please ping @cornbread2100 in the official support server (https://discord.gg/3u2fNRAMAN)'
      );
  ...
  let showingOldPlayers = true;

  let buttons = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(randomizeID).setLabel('↻').setStyle(ButtonStyle.Primary)
  );
  if (hasOldPlayers) {
  ...
  let embed = createEmbed(server, index, recentServers);
  await interaction.editReply({ content: '', embeds: [embed], components: [buttons] });

  buttonCallbacks[randomizeID] = async (interaction) =>
    execute(
      interaction,
      buttonCallbacks,
      client,
  ...
  buttonCallbacks[oldPlayersID] = async (interaction) => {
    if (interaction.user.id != user.id)
      return interaction.reply({
        content: "That's another user's command, use /random to create your own",
        ephemeral: true,
      });
    ...
    if (playerList == null) {
      playerList = await (
        await fetch(`${config.api}/playerHistory?ip=${server.ip}&port=${server.port}`)
      ).json();
      playerList.sort((a, b) => b.lastSession - a.lastSession);
    }
    lastButtonPress = new Date();
    showingOldPlayers = !showingOldPlayers;
    (buttons.components[1] as ButtonBuilder).setLabel(
      showingOldPlayers ? 'Online Players' : 'Player History'
    );
