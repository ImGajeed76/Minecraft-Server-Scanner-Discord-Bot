...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  const ip =
    interaction.options.getInteger('port') == null
      ? interaction.options.getString('ip').split(':')[0]
      : interaction.options.getString('ip');
  const port =
    interaction.options.getInteger('port') == null
      ? interaction.options.getString('ip').split(':')[1] || 25565
      : interaction.options.getInteger('port');
  await interaction.reply(`Pinging \`${ip}:${port}\`, please wait...`);

  const text = await (
    await fetch(`https://ping.cornbread2100.com/ping?ip=${ip}&port=${port}`)
  ).text();
  if (text == 'Error: timeout') {
    let errorEmbed = new EmbedBuilder().setColor('#ff0000').addFields({
      name: 'Timeout',
      value:
        'If you know this server is online, ping @cornbread2100 in the official support server (https://discord.gg/3u2fNRAMAN)',
    });
    ...
    let newEmbed = createEmbed(response, ip, port);
