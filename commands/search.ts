...
function createEmbed(server) {
  ...
  return new EmbedBuilder()
    ...
    .setThumbnail(
      `https://ping.cornbread2100.com/favicon?ip=${server.ip}&port=${server.port}&errors=false`
    )
    .addFields(
      ...
      {
        name: 'Discovered',
        value: `<t:${server.discovered}:${new Date().getTime() / 1000 - server.discovered > 86400 ? 'D' : 'R'}>`,
      },
      {
        name: 'Last Seen',
        value: `<t:${server.lastSeen}:${new Date().getTime() / 1000 - server.lastSeen > 86400 ? 'D' : 'R'}>`,
      },
      {
        name: 'Country: ',
        value: `${server.geo.country == null ? 'Unknown' : `:flag_${server.geo.country.toLowerCase()}: ${server.geo.country}`}`,
      },
      { name: 'Organization: ', value: server.org == null ? 'Unknown' : server.org },
      {
        name: 'Auth',
        value: server.cracked == true ? 'Cracked' : server.cracked == false ? 'Premium' : 'Unknown',
      },
      {
        name: 'Whitelist',
        value:
  ...
  if (showingOldPlayers != null || server.players?.hasPlayerSample) {
    buttons.addComponents(
      new ButtonBuilder()
        ...
        .setCustomId(
          `search-${showingOldPlayers ? 'online' : 'history'}-${server.ip}:${server.port}`
        )
        .setDisabled(loading)
    );
  }
...
const displayIp = (server) =>
  `${cleanIp(parseInt(server.ip))}${server.port == 25565 ? '' : `:${server.port}`}`;
...
function createList(servers, currentEmbed, totalResults, minimal) {
  const embed = new EmbedBuilder()
    .setColor('#02a337')
    .setTitle(
      `Results ${thousandsSeparators(currentEmbed + 1)}-${thousandsSeparators(currentEmbed + servers.length)}/${thousandsSeparators(totalResults)}`
    )
    .setAuthor({
      name: 'MC Server Scanner',
      iconURL:
  ...
  for (let i = 1; i < servers.length; i++) {
    if (displayIp(servers[i]).length > longest.server)
      longest.server = displayIp(servers[i]).length;
    if (displayVersion(servers[i].version).length > longest.version)
      longest.version = displayVersion(servers[i].version).length;
  }
  ...
  for (let i = 0; i < servers.length; i++) {
    ...
    if (!minimal)
      description += ` \`${displayVersion(servers[i].version)}${' '.repeat(longest.version - displayVersion(servers[i].version).length)}\` Pinged <t:${servers[i].lastSeen}:R>`;
  }
...
export const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((option) => option.setName('page').setDescription('Skips to a page of results'))
  .addStringOption((option) =>
    option.setName('playercount').setDescription('A range of online player counts')
  )
  .addIntegerOption((option) =>
    option.setName('playercap').setDescription("The server's maximum player capacity")
  )
  .addBooleanOption((option) =>
    option.setName('isfull').setDescription('whether or not the server is full')
  )
  .addStringOption((option) =>
    option
      .setName('player')
  ...
  .addStringOption((option) =>
    option.setName('version').setDescription('The version of the server')
  )
  .addIntegerOption((option) =>
    option.setName('protocol').setDescription('The protocol version of the server')
  )
  .addBooleanOption((option) =>
    option.setName('hasimage').setDescription('Whether or not the server has a custom favicon')
  )
  .addStringOption((option) =>
    option.setName('description').setDescription('The description of the server')
  )
  .addBooleanOption((option) =>
    option
  ...
  .addIntegerOption((option) =>
    option
      .setName('seenafter')
      .setDescription(
        "The oldest time a server can be last seen (this doesn't mean it's offline, use /help for more info)"
      )
      .setAutocomplete(true)
  )
  .addStringOption((option) =>
    option.setName('iprange').setDescription("The ip subnet a server's ip has to be within")
  )
  .addStringOption((option) =>
    option.setName('excluderange').setDescription("The ip subnet a server's ip cannot be within")
  )
  .addIntegerOption((option) =>
    option.setName('port').setDescription('The port the server is hosted on')
  )
  .addStringOption((option) =>
    option
      .setName('country')
  ...
  .addBooleanOption((option) =>
    option.setName('cracked').setDescription('Whether or not the server is cracked (offline mode)')
  )
  .addBooleanOption((option) =>
    option.setName('whitelisted').setDescription('Whether or not the server has a whitelisted')
  )
  .addBooleanOption((option) =>
    option.setName('vanilla').setDescription('Whether or not the server is vanilla')
  );
  ...
  switch (focusedValue.name) {
    case 'seenafter':
      await interaction.respond([
        { name: '1 hour ago', value: Math.round(new Date().getTime() / 1000) - 3600 },
        { name: '6 hours ago', value: Math.round(new Date().getTime() / 1000) - 21600 },
        { name: '1 day ago', value: Math.round(new Date().getTime() / 1000) - 86400 },
      ]);
      break;
    case 'country':
      await interaction.respond(
        countryCodes
          ...
          .splice(0, 25)
          .map((choice) => ({ name: choice.name, value: choice.code }))
      );
      break;
    case 'org':
      await interaction.respond(
  ...
  switch (id) {
    case 'info': {
      ...
      await interaction.reply({
        embeds: [createEmbed(server)],
        components: [createButtons(server, null, false)],
      });
      break;
    }
    case 'history': {
      ...
      await interaction.update({
        embeds: [embed],
        components: [createButtons({ ip, port }, true, true)],
      });
      const playerList: any[] = (await (
        await fetch(`${config.api}/playerHistory?ip=${ip}&port=${port}`)
      ).json()) as any[];
      playerList.sort((a, b) => b.lastSession - a.lastSession);
      let playerCounts = embed.data.fields[4].value.split('\n')[0];
      embed.data.fields[4].value = displayPlayers(
        {
          players: { online: playerCounts.split('/')[0], max: playerCounts.split('/')[1] },
          lastSeen: embed.data.fields[6].value.split(':')[1],
        },
        playerList,
        true
      );
      await interaction.editReply({
        embeds: [embed],
        components: [createButtons({ ip, port }, true)],
      });
      break;
    }
    case 'online': {
      ...
      await interaction.update({
        embeds: [embed],
        components: [createButtons({ ip, port }, false, true)],
      });
      const playerList: any[] = (await (
        await fetch(`${config.api}/playerHistory?ip=${ip}&port=${port}`)
      ).json()) as any[];
      ...
      await interaction.editReply({
        embeds: [embed],
        components: [createButtons({ ip, port }, false)],
      });
      break;
    }
  }
...
export async function execute(interaction: ChatInputCommandInteraction, buttonCallbacks) {
  ...
  if (interaction.guild?.id == '1222761600860291163') {
    const newEmbed = new EmbedBuilder()
      ...
      .setDescription(
        'Using this bot to grief Minecraft servers is strictly prohibited. This incident has been reported.'
      )
    ...
    function updateButtons() {
      ...
      if (totalResults > 10) {
        buttons.addComponents(
          ...
          new ButtonBuilder().setCustomId(nextResultID).setLabel('▶').setStyle(ButtonStyle.Success)
        );
      } else {
        buttons.addComponents(
          new ButtonBuilder()
      ...
      if (`${config.api}/servers?limit=10&skip=${currentEmbed}&${args}`.length <= 512) {
        buttons.addComponents(
          new ButtonBuilder()
            ...
            .setStyle(ButtonStyle.Link)
            .setURL(
              `${config.displayApi || config.api}/servers?limit=10&skip=${currentEmbed}&${args}`
            )
        );
      }
      ...
      for (
        let i = 0;
        i < (totalResults - currentEmbed < 5 ? totalResults - currentEmbed : 5);
        i++
      ) {
      ...
      for (
        let i = 5;
        i < (totalResults - currentEmbed < 10 ? totalResults - currentEmbed : 10);
        i++
      ) {
      ...
      buttonCallbacks[lastResultID] = async (interaction) => {
        if (interaction.user.id != user.id)
          return interaction.reply({
            content: "That's another user's command, use /search to create your own",
            ephemeral: true,
          });
        ...
        servers = await (
          await fetch(`${config.api}/servers?limit=10&skip=${currentEmbed}&${args}`)
        ).json();
        updateButtons();
        let newEmbed = createList(servers, currentEmbed, totalResults, minimal);
        await interaction.editReply({
          embeds: [newEmbed],
          components: [buttons, infoButtons, infoButtons2].filter((a) => a.components.length > 0),
        });
      };
  ...
  if (interaction.options.getInteger('page') != null)
    currentEmbed = interaction.options.getInteger('page') - 1;
  let playerCount;
  let minOnline;
  let maxOnline;
  ...
  argumentList += `\n- **${
    sort == 'none'
      ...
      : `sorted by ${
          {
            lastSeen: 'Last Ping',
            discovered: 'Dicovery Date',
          }[sort.split(':')[0]]
        } (${{ a: 'ascending', d: 'descending' }[sort.split(':')[1]]})`
  }**`;
  ...
  if (sort != 'none') {
    ...
    args.append('descending', String(sort.split(':')[1] == 'd'));
  }
  if (minOnline == maxOnline) {
    if (minOnline != null) args.append('playerCount', minOnline);
  } else {
    if (minOnline != null) args.append('minPlayers', minOnline);
    if (maxOnline != null) args.append('maxPlayers', maxOnline);
  }
  if (playerCap != null) args.append('playerLimit', String(playerCap));
  if (isFull != null) args.append('full', String(isFull));
  ...
  if (protocol != null) args.append('protocol', String(protocol));
  if (hasImage != null) args.append('hasFavicon', String(hasImage));
  if (description != null) {
    let segments = [''];
    let escaped = false;
    ...
    let quotes = segments.filter(
      (a, i) => i % 2 == 1 && (i != segments.length - 1 || segments.length % 2 == 1)
    );
    if (quotes.length > 0) args.append('description', `%${quotes.join('%')}%`);
    if (
      segments
        .filter((a, i) => i % 2 == 0 || !(i != segments.length - 1 || segments.length % 2 == 1))
        .join('').length > 0
    )
      args.append('descriptionVector', segments.join(''));
  }
  if (hasPlayerList != null) args.append('hasPlayerSample', String(hasPlayerList));
  if (seenAfter != null) args.append('seenAfter', String(seenAfter));
  if (ipRange != null) {
    let minIp = [];
    let maxIp = [];
    for (let range of ipRange.split(',')) {
      ...
      ip = ip
        .split('.')
        .reverse()
        .map((a, i) => parseInt(a) * 256 ** i)
        .reduce((a, b) => a + b, 0)
        .toString();
      let ipNum = parseInt(ip);
      let subnetNum = subnet == null ? 32 : parseInt(subnet);
      if (subnet == null || subnetNum >= 32) args.append('ip', ip);
      else {
        minIp.push((ipNum & ~((1 << (32 - subnetNum)) - 1)) >>> 0);
        maxIp.push((ipNum | ((1 << (32 - subnetNum)) - 1)) >>> 0);
      }
    }
  ...
  if (excludeRange != null) {
    for (let range of excludeRange.split(',')) {
      ...
      let ipNum = parseInt(ip);
      let subnetNum = subnet == null ? 32 : parseInt(subnet);
      let excludeMinIp = (subnetNum & ~((1 << (32 - subnetNum)) - 1)) >>> 0;
      let excludeMaxIp = (subnetNum | ((1 << (32 - subnetNum)) - 1)) >>> 0;
      args.append(
        'minIp',
        JSON.stringify(
          [
            excludeMaxIp <= 0 ? null : 0,
            excludeMaxIp >= 4294967295 ? null : excludeMaxIp + 1,
          ].filter((a) => a != null)
        )
      );
      args.append(
        'maxIp',
        JSON.stringify([excludeMaxIp <= 0 ? null : excludeMinIp - 1].filter((a) => a != null))
      );
    }
  }
  if (port != null) args.append('port', String(port));
  if (country != null) args.append('country', country);
  if (org != null) args.append('org', org);
  if (cracked != null) args.append('cracked', String(cracked));
  if (whitelisted != null) args.append('whitelisted', String(whitelisted));
  if (vanilla != null) args.append('vanilla', String(vanilla));
  ...
  if (servers.length > 0) {
    ...
    new Promise(async (resolve) =>
      resolve(await (await fetch(`${config.api}/count?${args}`)).json())
    ).then((response) => (totalResults = response));
    ...
    await interaction.editReply({
      content: '',
      embeds: [new EmbedBuilder().setColor('#02a337').setTitle('No matches could be found')],
      components: [
