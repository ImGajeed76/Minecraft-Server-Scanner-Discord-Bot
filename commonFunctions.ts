export const minecraftToAnsi = (text: string) => {
...
export const getDescription = (description: any) => {
  ...
  if (typeof description != 'object') return getDescription({ text: String(description) });
  if (Array.isArray(description)) return description.reduce((a, b) => a + getDescription(b), '');
  description =
    String(description.text == null ? '' : description.text) +
    String(description.translate == null ? '' : description.translate) +
    (description.extra || []).reduce((a, b) => a + getDescription(b), '');
  description = minecraftToAnsi(description);
  return description;
};

export const getVersion = (rawVersion: any) => {
  let version: any = '';
...
export const cleanIp = (ip: number) =>
  ('0'.repeat(8 - ip.toString(16).length) + ip.toString(16))
    .match(/.{1,2}/g)!
    .map((a) => parseInt(a, 16))
    .join('.');

export const displayPlayers = (server: any, playerList?: any, showingOldPlayers?: boolean) => {
  let result = `${server.players.online}/${server.players.max}`;
  if (playerList == null) return result;
  if (
    playerList.length > 0 &&
    (showingOldPlayers || playerList.filter((a) => a.lastSession == server.lastSeen).length > 0)
  ) {
